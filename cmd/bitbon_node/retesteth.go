...
func (e *NoRewardEngine) accumulateRewards(config *params.ChainConfig, stateDB *state.DB, header *types.Header, uncles []*types.Header) {
	// Simply touch miner and uncle coinbase accounts
	reward := big.NewInt(0)
	for _, uncle := range uncles {
		stateDB.AddBalance(uncle.Coinbase, reward)
	}
	stateDB.AddBalance(header.Coinbase, reward)
}

func (e *NoRewardEngine) Finalize(chain consensus.ChainReader, header *types.Header, statedb *state.DB, txs []*types.Transaction,
	uncles []*types.Header) {
	if e.rewardsOn {
...
func (e *NoRewardEngine) FinalizeAndAssemble(chain consensus.ChainReader, header *types.Header, statedb *state.DB, txs []*types.Transaction,
...
func (e *NoRewardEngine) CalcDifficulty(chain consensus.ChainReader, t uint64, parent *types.Header) *big.Int {
	return e.inner.CalcDifficulty(chain, t, parent)
}
	...
	for address := range api.txSenders {
		...
		for nonce := statedb.GetNonce(address); ; nonce++ {
			if tx, ok := m[nonce]; ok {
				...
				receipt, err := core.ApplyTransaction( // nolint:govet
					api.chainConfig,
					api.blockchain,
					&api.author,
					gasPool,
...
func (api *RetestethAPI) GetLogHash(ctx context.Context, txHash common.Hash) (common.Hash, error) {
	receipt, _, _, _ := rawdb.ReadReceipt(api.ethDb, txHash, api.chainConfig) // nolint
	if receipt == nil {
...
func (api *RetestethAPI) AccountRange(ctx context.Context,
	...
	if parentHeader == nil || int(txIndex) >= len(block.Transactions()) {
		...
		for idx, tx := range block.Transactions() {
			...
			evmContext := core.NewEVMContext(msg, block.Header(), api.blockchain, nil)
			// Not yet the searched for transaction, execute on top of the current state
			vmenv := vm.NewEVM(evmContext, statedb, api.blockchain.Config(), vm.Config{})
...
func (api *RetestethAPI) StorageRangeAt(ctx context.Context,
	...
	for i := 0; /*i < int(maxResults) && */ it.Next(); i++ {
		if preimage := storageTrie.GetKey(it.Key); preimage != nil { //nolint
