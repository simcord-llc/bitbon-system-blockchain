...
import (
	...
	"github.com/simcord-llc/bitbon-system-blockchain/common/compiler"
)
...
var (
	...
	solcAllowFlag = cli.StringFlag{
		Name:  "solc-allow-paths",
		Usage: "Solidity compiler --allow-paths argument values. Allow a given path for imports. A list of paths can be supplied by separating them with a comma.",
		Value: "",
	}
...
func abigen(c *cli.Context) error {
	...
	if c.GlobalString(abiFlag.Name) != "" {
		...
		case c.GlobalIsSet(solFlag.Name):
			var args []string
			ss := c.GlobalString(solcAllowFlag.Name)
			if ss != "" {
				args = []string{
					"--allow-paths",
					solcAllowFlag.Value,
			...
			contracts, err = compiler.CompileSolidity(solcFlag.Value, args, solFlag.Value)
			if err != nil {
