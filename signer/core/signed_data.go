...
func parseInteger(encType string, encValue interface{}) (*big.Int, error) {
	...
	if encType == "int" || encType == uintValue {
		length = 256
	} else {
		lengthStr := ""
		if strings.HasPrefix(encType, uintValue) {
			lengthStr = strings.TrimPrefix(encType, uintValue)
		} else {
...
func (typedData *TypedData) EncodePrimitiveValue(encType string, encValue interface{}, depth int) ([]byte, error) {
	...
	if strings.HasPrefix(encType, byteValue) {
		lengthStr := strings.TrimPrefix(encType, byteValue)
	...
	if strings.HasPrefix(encType, "int") || strings.HasPrefix(encType, uintValue) {
...
func (api *SignerAPI) EcRecover(ctx context.Context, data hexutil.Bytes, sig hexutil.Bytes) (common.Address, error) {
	...
	//
	// https://github.com/simcord-llc/bitbon-system-blockchain/wiki/Management-APIs#personal_ecRecover
	if len(sig) != 65 {
...
func UnmarshalValidatorData(data interface{}) (ValidatorData, error) {
	...
	addr, ok := raw[addressValue].(string)
	if !ok {
...
func formatPrimitiveValue(encType string, encValue interface{}) (string, error) {
	...
	if strings.HasPrefix(encType, uintValue) || strings.HasPrefix(encType, "int") {
...
func (domain *TypedDataDomain) validate() error {
	...
	if domain.Name == "" && domain.Version == "" && domain.VerifyingContract == "" && domain.Salt == "" {
