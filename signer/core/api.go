...
import (
	...
	"github.com/simcord-llc/bitbon-system-blockchain/accounts/scwallet"
	"github.com/simcord-llc/bitbon-system-blockchain/accounts/usbwallet"
	...
	"github.com/simcord-llc/bitbon-system-blockchain/signer/storage"
)
...
func (api *SignerAPI) List(ctx context.Context) ([]common.Address, error) {
	accs := make([]accounts.Account, 0, len(api.am.Wallets()))
...
func logDiff(original *SignTxRequest, response *SignTxResponse) bool {
	modified := false
	if f0, f1 := original.Transaction.From, response.Transaction.From; !reflect.DeepEqual(f0, f1) {
		log.Info("Sender-account changed by UI", "was", f0, "is", f1)
		modified = true
	}
	if t0, t1 := original.Transaction.To, response.Transaction.To; !reflect.DeepEqual(t0, t1) {
		log.Info("Recipient-account changed by UI", "was", t0, "is", t1)
		modified = true
	}
	if g0, g1 := original.Transaction.Gas, response.Transaction.Gas; g0 != g1 {
		modified = true
		log.Info("Gas changed by UI", "was", g0, "is", g1)
	}
	if g0, g1 := big.Int(original.Transaction.GasPrice), big.Int(response.Transaction.GasPrice); g0.Cmp(&g1) != 0 {
		modified = true
		log.Info("GasPrice changed by UI", "was", g0, "is", g1)
	}
	if v0, v1 := big.Int(original.Transaction.Value), big.Int(response.Transaction.Value); v0.Cmp(&v1) != 0 {
		modified = true
		log.Info("Value changed by UI", "was", v0, "is", v1)
	}
	if d0, d1 := original.Transaction.Data, response.Transaction.Data; d0 != d1 {
		d0s := ""
		d1s := ""
		if d0 != nil {
	...
	if n0, n1 := original.Transaction.Nonce, response.Transaction.Nonce; n0 != n1 {
		modified = true
