...
type Config struct {
	...
	Notify    []string       `toml:",omitempty"` // HTTP URL list to be notified of new work packages(only in ethash).
...
// nolint:staticcheck
func New(eth Backend, config *params.ChainConfig, minerConfig *Config, mux *event.TypeMux,
	engine consensus.Engine, isLocalBlock func(block *types.Block) bool, q *quorum.Quorum) *Miner {
	miner := &Miner{
		eth:      eth,
		mux:      mux,
		engine:   engine,
		exitCh:   make(chan struct{}),
		worker:   newWorker(minerConfig, config, engine, eth, mux, isLocalBlock, true),
		canStart: 1,
		quorum:   q,
	}
	go miner.update()
...
func (miner *Miner) Start(coinbase common.Address) {
	miner.quorum.MarkMinerCandidate(true)
...
func (miner *Miner) Stop() {
	miner.quorum.MarkMinerCandidate(false)
	miner.worker.stop()
	atomic.StoreInt32(&miner.shouldStart, 0)
}
...
func (miner *Miner) Pending() (*types.Block, *state.DB) {
