...
const descriptionMsg = "An error was encountered when saving and verifying the keystore file. \n" +
	"This indicates that the keystore is corrupted. \n" +
	"The corrupted file is stored at \n%v\n" +
	"Please file a ticket at:\n\n" +
	"https://github.com/simcord-llc/bitbon-system-blockchain/issues." +
	"The error was : %s"
...
func (ks keyStorePassphrase) StoreKey(filename string, key *Key, auth string) error {
	...
	if !ks.skipKeyFileVerification {
		...
		if err != nil {
			return fmt.Errorf(descriptionMsg, tmpName, err) // nolint
		}
	}
...
func EncryptKey(key *Key, auth string, scryptN, scryptP int) ([]byte, error) {
	...
	keyJSONV3 := encryptedKeyJSONV3{
		hex.EncodeToString(key.Address[:]),
		cryptoStruct,
		key.ID.String(),
		version,
	}
	return json.Marshal(keyJSONV3)
}
