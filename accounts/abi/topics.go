...
func MakeTopics(query ...[]interface{}) ([][]common.Hash, error) { // nolint:gocyclo,funlen
...
func genIntType(rule int64, size uint) []byte {
	...
	if rule < 0 {
		...
		topic = [common.HashLength]byte{255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255,
...
func parseTopicWithSetter(fields Arguments, topics []common.Hash, setter func(Argument, interface{})) error {
	...
	for i := range fields {
		...
		default:
			var err error
			reconstr, err = toGoType(0, fields[i].Type, topics[i].Bytes())
			if err != nil {
				return err
			}
		}
		// Use the setter function to store the value
		setter(fields[i], reconstr)
	}
