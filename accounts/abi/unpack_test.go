...
func TestUnpack(t *testing.T) {
	for i, test := range packUnpackTests {
		t.Run(strconv.Itoa(i)+" "+test.def, func(t *testing.T) {
			...
			if err != nil {
				t.Errorf("test %d (%v) failed: %v", i, test.def, err) // nolint:scopelint
				return
			}
...
func methodMultiReturn(requireAssertions *require.Assertions) (ABI, []byte, methodMultiOutput) {
	const definition = `[
	{ "name" : "multi", "type": "function", "outputs": [ { "name": "Int", "type": "uint256" }, { "name": "String", "type": "string" } ] }]`
	var expected = methodMultiOutput{big.NewInt(1), "hello"}

	abi, err := JSON(strings.NewReader(definition))
	requireAssertions.NoError(err)
...
func TestMethodMultiReturn(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.error == "" {
				requireAssertion.Nil(err, "Should be able to unpack method outputs.")
				requireAssertion.Equal(tc.expected, tc.dest)
			} else {
				requireAssertion.EqualError(err, tc.error)
			}
		})
	}
}
