...
type Wallet struct {
	...
	deriveNextPaths []accounts.DerivationPath // paths for account auto-discovery (multiple bases supported)
	deriveNextAddrs []common.Address          // account addresses for auto-discovery (multiple bases supported)
...
func transmit(card *pcsc.Card, command *commandAPDU) (*responseAPDU, error) {
	...
	if response.Sw1 != sw1Ok {
		return nil, fmt.Errorf("unexpected insecure response status Cla=0x%x, Ins=0x%x, Sw=0x%x%x",
			command.Cla, command.Ins, response.Sw1, response.Sw2)
	}
...
func (w *Wallet) selfDerive() { // nolint
		...
		for i := 0; i < len(nextAddrs); i++ {
			for empty := false; !empty; {
				...
				if _, known := pairing.Accounts[nextAddrs[i]]; !known || !empty || nextAddrs[i] != w.deriveNextAddrs[i] {
					w.log.Info("Smartcard wallet discovered new account", "address", nextAddrs[i],
						"path", path, "balance", balance, "nonce", nonce)
				}
