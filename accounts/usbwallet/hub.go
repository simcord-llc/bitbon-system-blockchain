...
func NewLedgerHub() (*Hub, error) {
	return newHub(LedgerScheme, 0x2c97, []uint16{
		...
		// Upcoming product IDs:
		// https://www.ledger.com/2019/05/17/windows-10-update-sunsetting-u2f-tunnel-transport-for-ledger-devices/
...
func NewTrezorHubWithWebUSB() (*Hub, error) {
	return newHub(TrezorScheme, 0x1209, []uint16{0x53c1 /* Trezor WebUSB */},
		0xffff /* No usage id on webusb, don't match unset (0) */, 0, newTrezorDriver)
}

// newHub creates a new hardware wallet manager for generic USB devices.
func newHub(scheme string, vendorID uint16, productIDs []uint16, usageID uint16,
	endpointID int, makeDriver func(log.Logger) driver) (*Hub, error) {
	if !usb.Supported() {
...
func (hub *Hub) refreshWallets() { // nolint:gocyclo,funlen
	...
	if err != nil {
		...
		log.Debug("Failed to enumerate USB devices", "hub", hub.scheme,
	...
	for idx := range infos {
		for _, id := range hub.productIDs {
			...
			if infos[idx].ProductID == id && (infos[idx].UsagePage == hub.usageID || infos[idx].Interface == hub.endpointID) {
				devices = append(devices, infos[idx])
				break
			}
		}
	}
	...
	var (
		...
		events  = make([]accounts.WalletEvent, 0, len(hub.wallets))
	)

	for idx := range devices {
		url := accounts.URL{Scheme: hub.scheme, Path: devices[idx].Path}
		...
		if len(hub.wallets) == 0 || hub.wallets[0].URL().Cmp(url) > 0 {
			...
			wallet := &wallet{hub: hub, driver: hub.makeDriver(logger), url: &url, info: devices[idx], log: logger}
