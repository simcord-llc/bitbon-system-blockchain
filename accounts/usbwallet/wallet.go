...
type driver interface {
	...
	SignTx(path accounts.DerivationPath,
...
type wallet struct {
	...
	deriveNextPaths []accounts.DerivationPath // for account auto-discovery (multiple bases supported)
	deriveNextAddrs []common.Address          // addresses for auto-discovery (multiple bases supported)
		...
		for i := 0; i < len(nextAddrs); i++ {
			for empty := false; !empty; {
				...
				if _, known := w.paths[nextAddrs[i]]; !known || (!empty && nextAddrs[i] == w.deriveNextAddrs[i]) {
					w.log.Info("USB wallet discovered new account",
						"address", nextAddrs[i], "path", path, "balance", balance, "nonce", nonce)
				}
...
func (w *wallet) SignDataWithPassphrase(account accounts.Account,
