...
func CompileSolidity(solc string, args []string, sourcefiles ...string) (map[string]*Contract, error) {
	...
	args = append(args, append(s.makeArgs(), "--")...)
...
func ParseCombinedJSON(combinedJSON []byte, source, languageVersion, compilerVersion, compilerOptions string) (map[string]*Contract, error) {
	...
	for name := range output.Contracts {
		...
		var abi interface{}
		if err := json.Unmarshal([]byte(output.Contracts[name].Abi), &abi); err != nil {
			return nil, fmt.Errorf("solc: error reading abi definition (%v)", err)
		}
		var userdoc, devdoc interface{}
		json.Unmarshal([]byte(output.Contracts[name].Userdoc), &userdoc) // nolint:errcheck
		json.Unmarshal([]byte(output.Contracts[name].Devdoc), &devdoc)   // nolint:errcheck

		contracts[name] = &Contract{
			Code:        "0x" + output.Contracts[name].Bin,
			RuntimeCode: "0x" + output.Contracts[name].BinRuntime,
			Hashes:      output.Contracts[name].Hashes,
			Info: ContractInfo{
				Source:          source,
				Language:        "Solidity",
				...
				SrcMap:          output.Contracts[name].SrcMap,
				SrcMapRuntime:   output.Contracts[name].SrcMapRuntime,
				AbiDefinition:   abi,
				UserDoc:         userdoc,
				DeveloperDoc:    devdoc,
				Metadata:        output.Contracts[name].Metadata,
			},
		}
	}
