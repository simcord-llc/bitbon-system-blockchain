...
func (n *ExecNode) ServeRPC(clientConn *websocket.Conn) error {
	conn, _, err := websocket.DefaultDialer.Dial(n.wsAddr, nil) // nolint:bodyclose
	if err != nil {
		return err
	}
...
func (n *ExecNode) Snapshots() (map[string][]byte, error) {
	...
	err := n.client.Call(&snapshots, "simulation_snapshot")
...
func execP2PNode() {
	...
	if _, err := http.Post(statusURL, "application/json", bytes.NewReader(statusJSON)); err != nil { // nolint
	...
	for _, name := range serviceNames {
		...
		constructor := func(nodeCtx *node.ServiceContext) (node.Service, error) {
			...
			if conf.Snapshots != nil {
				ctx.Snapshot = conf.Snapshots[sName]
			}
