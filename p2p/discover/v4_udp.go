...
import (
	...
	"github.com/simcord-llc/bitbon-system-blockchain/p2p/discover/v4wire"
...
func (t *UDPv4) Resolve(n *enode.Node) *enode.Node {
	...
	for _, rn := range result {
		if rn.ID() == n.ID() {
			if rn, err := t.RequestENR(rn); err == nil { // nolint:govet
				return rn
			}
		}
	}
...
func (t *UDPv4) send(toaddr *net.UDPAddr, toid enode.ID, req v4wire.Packet) ([]byte, error) { // nolint
	packet, hash, err := v4wire.Encode(t.priv, req)
	if err != nil {
		return hash, err
	}
