...
import (
	...
	"github.com/simcord-llc/bitbon-system-blockchain/internal/testlog"
...
func runDialTest(t *testing.T, config dialConfig, rounds []dialTestRound) {
	...
	for i := range rounds {
		...
		for _, c := range rounds[i].peersAdded {
		...
		for _, id := range rounds[i].peersRemoved {
			c := peers[id]
			if c == nil {
		...
		resolver.setAnswers(rounds[i].wantResolves)
		if rounds[i].update != nil {
			rounds[i].update(dialsched)
		}
		iterator.addNodes(rounds[i].discovered)

		// Unblock dialTask goroutines.
		if err := dialer.completeDials(rounds[i].succeeded, nil); err != nil {
			t.Fatalf("round %d: %v", i, err)
		}
		for range rounds[i].succeeded {
			conn := <-setupCh
			peers[conn.node.ID()] = conn
		}
		if err := dialer.completeDials(rounds[i].failed, errors.New("oops")); err != nil {
			t.Fatalf("round %d: %v", i, err)
		}

		// Wait for new tasks.
		if err := dialer.waitForDials(rounds[i].wantNewDials); err != nil {
			t.Fatalf("round %d: %v", i, err)
		}
