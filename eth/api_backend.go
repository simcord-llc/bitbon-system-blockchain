...
func (b *EthAPIBackend) StateAndHeaderByNumber(ctx context.Context, number rpc.BlockNumber) (*state.DB, *types.Header, error) {
	// Pending state is only known by the miner
	if number == rpc.PendingBlockNumber {
		block, s := b.eth.miner.Pending()
		return s, block.Header(), nil
	}
...
func (b *EthAPIBackend) StateAndHeaderByNumberOrHash(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash) (*state.DB, *types.Header, error) {
	if blockNr, ok := blockNrOrHash.Number(); ok {
...
func (b *EthAPIBackend) GetEVM(_ context.Context, msg core.Message, stateDB *state.DB, header *types.Header) (*vm.EVM, func() error, error) {
	vmError := func() error { return nil }

	ctx := core.NewEVMContext(msg, header, b.eth.BlockChain(), nil)
	return vm.NewEVM(ctx, stateDB, b.eth.blockchain.Config(), *b.eth.blockchain.GetVMConfig()), vmError, nil
}
...
func (b *EthAPIBackend) BloomStatus() (uint64, uint64) { // nolint
