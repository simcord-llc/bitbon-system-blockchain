...
func (q *queue) expire(timeout time.Duration, pendPool map[string]*fetchRequest, taskQueue *prque.Prque, timeoutMeter metrics.Meter) map[string]int {
	...
	for id, request := range pendPool {
		if time.Since(request.Time) > timeout { // nolint
	...
	if accepted {
		for i, header := range headers[1:] {
			...
			if headers[i].Hash() != header.ParentHash {
				log.Warn("Header broke chain ancestry", "peer", id, "curNumber", header.Number, "parentNumber", headers[i].Number, "curHash", hash.Hex(), "curParentHash", header.ParentHash.Hex(), "parentHash", headers[i].Hash().Hex(), "index(i)", i)
				accepted = false
				break
			}
		}
...
func (q *queue) deliver(id string, taskPool map[common.Hash]*types.Header, taskQueue *prque.Prque,
	...
	for i, header := range request.Headers {
		...
		if index >= len(q.resultCache) || index < 0 || q.resultCache[index] == nil {
			log.Error("failure = errInvalidChain")
