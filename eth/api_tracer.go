...
import (
	...
	"github.com/simcord-llc/bitbon-system-blockchain/eth/tracers"
...
func (api *PrivateDebugAPI) computeStateDB(block *types.Block, reexec uint64) (*state.DB, error) {
...
// be tracer dependent.
func (api *PrivateDebugAPI) traceTx(ctx context.Context, message core.Message, vmctx vm.Context, statedb *state.DB, config *TraceConfig) (interface{}, error) { // nolint:gocyclo
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash common.Hash, txIndex int, reexec uint64) (core.Message, vm.Context, *state.DB, error) {
	// Create the parent state database
	...
	for idx, tx := range block.Transactions() {
		...
		vmenv := vm.NewEVM(ctx, statedb, api.eth.blockchain.Config(), vm.Config{})
