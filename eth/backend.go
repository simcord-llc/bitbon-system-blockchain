...
type Ethereum struct {
	...
	eventMux       *event.TypeMux // nolint:staticcheck
...
func New(ctx *node.ServiceContext, config *Config) (*Ethereum, error) {
	...
	err := ctx.Service(&q)
	if err != nil {
		return nil, errors.Wrap(err, "error getting quorum service from context")
	}
	...
	eth.miner = miner.New(eth, chainConfig, &config.Miner, eth.EventMux(), eth.engine, eth.isLocalBlock, eth.quorum)
...
func CreateConsensusEngine(ctx *node.ServiceContext, chainConfig *params.ChainConfig, config *ethash.Config, notify []string, noverify bool, db ethdb.Database) consensus.Engine {
	createEthashEngine := func() *ethash.Ethash {
		...
		default:
			...
			}, notify, noverify, chainConfig.MaxGenDAGBlock)
			engine.SetThreads(-1) // Disable CPU mining
			return engine
		}
	}

	// If community proof-of-stake is requested, set it up
	if chainConfig.Quorum != nil {
		var q *quorum.Quorum
		err := ctx.Service(&q)
		if err != nil {
		...
		cposBlock := chainConfig.CposBlock
		if cposBlock != nil && cposBlock.Cmp(big.NewInt(0)) == 0 {
			cposBlock = nil
		}

		q.SetEngine(quorumEngine.New(chainConfig.Quorum, db, q, cposBlock))
		return q.Engine()
...
func (s *Ethereum) Etherbase() (eb common.Address, err error) {
	...
	if wallets := s.AccountManager().Wallets(); len(wallets) > 0 {
		if acc := wallets[0].Accounts(); len(acc) > 0 {
			etherbase := acc[0].Address
...
func (s *Ethereum) shouldPreserve(block *types.Block) bool {
	...
	if _, ok := s.engine.(*quorumEngine.QuorumEngine); ok {
		return false
	}
...
func (s *Ethereum) StartMining(threads int) error {
	...
	if !s.IsMining() {
		...
		if q, ok := s.engine.(*quorumEngine.QuorumEngine); ok {
			nodeKey := s.p2pServer.PrivateKey
			addr := crypto.PubkeyToAddress(nodeKey.PublicKey)
			q.Authorize(addr, func(hash []byte) ([]byte, error) {
				res, err := crypto.Sign(hash, nodeKey)
				if err != nil {
					return nil, err
				}
				return res, nil
...
func (s *Ethereum) EventMux() *event.TypeMux           { return s.eventMux } // nolint:staticcheck
