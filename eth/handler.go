...
func (pm *ProtocolManager) txBroadcastLoop() {
	...
	for {
		select {
		case txsEvent := <-pm.txsCh:
			// For testing purpose only, disable propagation
			if pm.broadcastTxAnnouncesOnly {
				pm.BroadcastTransactions(txsEvent.Txs, false)
				continue
			}
			pm.BroadcastTransactions(txsEvent.Txs, true)  // First propagate transactions to peers
			pm.BroadcastTransactions(txsEvent.Txs, false) // Only then announce to the rest
