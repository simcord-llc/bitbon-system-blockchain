...
// the network.
func (f *BlockFetcher) Notify(peer string, hash common.Hash, number uint64, t time.Time,
...
func (f *BlockFetcher) FilterHeaders(peer string, headers []*types.Header, t time.Time) []*types.Header {
	...
	select {
	case filter <- &headerFilterTask{peer: peer, headers: headers, time: t}:
	case <-f.quit:
		return nil
	}
	...
	for {
		...
		case <-fetchTimer.C:
			...
			for peer, hashes := range request {
				...
				fetchHeader, hashes := f.fetching[hashes[0]].fetchHeader, hashes // nolint
				go func() {
