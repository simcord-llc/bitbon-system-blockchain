...
func (f *TxFetcher) rescheduleTimeout(timer *mclock.Timer, trigger chan struct{}) { // nolint
	if *timer != nil {
		(*timer).Stop()
	}
	now := f.clock.Now()
...
func (f *TxFetcher) scheduleFetches(timer *mclock.Timer, timeout chan struct{}, whitelist map[string]struct{}) { // nolint
