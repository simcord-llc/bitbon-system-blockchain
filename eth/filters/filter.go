...
type Filter struct {
	...
	logs []*types.Log // slice of logs if filtering a given logs
...
// NewLogsFilter creates a new filter which directly inspects the contents of
// a given logs to figure out which of them are interesting or not.
func NewLogsFilter(backend Backend, logs []*types.Log, addresses []common.Address, topics [][]common.Hash) *Filter {
	// Create a generic filter and convert it into a logs filter
	filter := newFilter(backend, addresses, topics)
	filter.logs = logs
	return filter
}
...
func (f *Filter) Logs(ctx context.Context) ([]*types.Log, error) {
	// If we're doing already given logs filtering, execute and return
	if len(f.logs) > 0 {
		return FilterLogs(f.logs, f.addresses, f.topics), nil
	}
...
func FilterLogs(logs []*types.Log, addresses []common.Address, topics [][]common.Hash) []*types.Log {
	return filterLogs(logs, nil, nil, addresses, topics)
}
