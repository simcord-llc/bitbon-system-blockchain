...
import (
	...
	"github.com/simcord-llc/bitbon-system-blockchain/common/bitutil"
				...
				case request := <-eth.bloomRequests:
					...
					for i, section := range task.Sections {
						...
						if compVector, err := rawdb.ReadBloomBits(eth.chainDb, task.Bit, section, head); err == nil {
							if blob, decompressErr := bitutil.DecompressBytes(compVector, int(sectionSize/8)); decompressErr == nil {
								task.Bitsets[i] = blob
							} else {
								task.Error = decompressErr
							}
						} else {
							task.Error = err
						}
