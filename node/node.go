...
func New(conf *Config) (*Node, error) {
	...
	return &Node{
		...
		eventmux:          new(event.TypeMux), // nolint:staticcheck
		log:               conf.Logger,
	}, nil
}
...
func (n *Node) Start() error {
	...
	services, err := n.prepareServices()
	if err != nil {
		return err
	}
	...
	err = n.startServices(services, running)
	if err != nil {
		return err
	}
...
func (n *Node) startServices(services map[reflect.Type]Service, running *p2p.Server) error {
	started := make([]reflect.Type, 0, len(services))
	for kind, service := range services {
		// Start the next service, stopping all previous upon failure
		if err := service.Start(running); err != nil {
			for _, kind := range started {
				services[kind].Stop() // nolint
			}
			running.Stop()
...
func (n *Node) prepareServices() (map[reflect.Type]Service, error) {
	...
	for _, constructor := range n.serviceFuncs {
		...
		if _, exists := services[kind]; exists {
			return nil, &DuplicateServiceError{Kind: kind}
		}
...
func (n *Node) startRPC(services map[reflect.Type]Service) error {
	...
	if err := n.startHTTP(n.httpEndpoint, apis, n.config.HTTPModules, n.config.HTTPCors,
		n.config.HTTPVirtualHosts, n.config.HTTPTimeouts, n.config.WSOrigins); err != nil {
		n.stopIPC()
		n.stopInProc()
		return err
	}
...
func (n *Node) startHTTP(endpoint string, apis []rpc.API, modules, cors, vhosts []string,
	timeouts rpc.HTTPTimeouts, wsOrigins []string) error {
...
func (n *Node) stopHTTP() {
	if n.httpServer != nil {
		...
		n.httpServer.Shutdown(context.Background()) // nolint
...
func (n *Node) startWS(endpoint string, apis []rpc.API, modules, wsOrigins []string, exposeAll bool) error {
...
func (n *Node) stopWS() {
	if n.wsHTTPServer != nil {
		// Don't bother imposing a timeout here.
		n.wsHTTPServer.Shutdown(context.Background()) // nolint
...
func (n *Node) EventMux() *event.TypeMux { // nolint:staticcheck
	return n.eventmux
}
...
func (n *Node) OpenDatabaseWithFreezer(name string, cache, handles int,
	freezer, namespace string) (ethdb.Database, error) {
	if n.config.DataDir == "" {
		return rawdb.NewMemoryDatabase(), nil
	}
