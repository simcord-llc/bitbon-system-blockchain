	...
	for i, header := range headers {
		...
		if tally := snap.Tally[header.Coinbase]; tally.Votes > len(snap.Signers)/2 {
			if tally.Authorize {
				...
				for ix := 0; ix < len(snap.Votes); ix++ {
					if snap.Votes[ix].Signer == header.Coinbase {
						// Uncast the vote from the cached tally
						snap.uncast(snap.Votes[ix].Address, snap.Votes[ix].Authorize)

						// Uncast the vote from the chronological list
						snap.Votes = append(snap.Votes[:ix], snap.Votes[ix+1:]...)
			...
			for ix := 0; ix < len(snap.Votes); ix++ {
				if snap.Votes[ix].Address == header.Coinbase {
					snap.Votes = append(snap.Votes[:ix], snap.Votes[ix+1:]...)
					ix--
				}
			}
