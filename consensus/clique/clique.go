...
// rewards given.
func (c *Clique) Finalize(chain consensus.ChainReader, header *types.Header, state *state.DB, txs []*types.Transaction, uncles []*types.Header) {
...
func (c *Clique) FinalizeAndAssemble(chain consensus.ChainReader, header *types.Header, state *state.DB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt) (*types.Block, error) {
	// No block rewards in PoA, so the state remains as is and uncles are dropped
	header.Root = state.IntermediateRoot(chain.Config().IsEIP158(header.Number))
	header.UncleHash = types.CalcUncleHash(nil)
