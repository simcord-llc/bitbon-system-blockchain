...
var (
	...
	// lastEpochToGenerateDAG is a epoch number, after which ther is no need to generate DAG
	leMux                  sync.RWMutex // mutex to protect lastEpochToGenerateDAG
	lastEpochToGenerateDAG *uint64
)

func GetLastEpochToGenerateDAG() *uint64 {
	leMux.RLock()
	defer leMux.RUnlock()
	return lastEpochToGenerateDAG
}

func SetLastEpochToGenerateDAG(block *big.Int) {
	var lastEpoch *uint64
	if block != nil {
		epoch := block.Uint64() / epochLength
		log.Info("Last epoch DAG will be generated will be set to", "epoch", epoch)
		lastEpoch = &epoch
	}

	leMux.Lock()
	defer leMux.Unlock()
	lastEpochToGenerateDAG = lastEpoch
}
...
func newlru(what string, maxItems int, newFn func(epoch uint64) interface{}) *lru {
	...
	return &lru{what: what, new: newFn, cache: cache}
}
...
// packages.
func New(config Config, notify []string, noverify bool, maxBlockToGenerateDataset *big.Int) *Ethash {
	if config.Log == nil {
		config.Log = log.Root()
	}
	...
	SetLastEpochToGenerateDAG(maxBlockToGenerateDataset)
	log.Warn("Ethash consensus engine has been activated")
...
func getEpoch(block uint64) uint64 {
	lastEpoch := GetLastEpochToGenerateDAG()
	epoch := block / epochLength

	if lastEpoch != nil && epoch > *lastEpoch {
		epoch = *lastEpoch
	}
...
func (ethash *Ethash) cache(block uint64) *cache {
	...
	currentI, _ := ethash.caches.get(epoch)
