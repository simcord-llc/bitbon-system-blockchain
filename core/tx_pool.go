...
func (pool *TxPool) Stats() (int, int) { // nolint
	pool.mu.RLock()
	defer pool.mu.RUnlock()
...
func (pool *TxPool) add(tx *types.Transaction, local bool) (replaced bool, err error) {
	...
	if list := pool.pending[from]; list != nil && list.Overlaps(tx) {
		return pool.replaceExistingTransaction(from, tx, hash, list)
	}
...
func (pool *TxPool) replaceExistingTransaction(
	from common.Address, tx *types.Transaction,
	hash common.Hash, list *txList) (bool, error) {
...
func (pool *TxPool) addTxs(txs []*types.Transaction, local, s bool) []error {
	...
	for _, tx := range news {
		types.Sender(pool.signer, tx) // nolint:errcheck
	}
	...
	if pending := pool.pending[addr]; pending != nil {
		if removed, invalids := pending.Remove(tx); removed {
			...
			for _, tx := range invalids {
				pool.enqueueTx(tx.Hash(), tx) // nolint:errcheck
			}
...
func (pool *TxPool) reset(oldHead, newHead *types.Header) {
	...
	if oldHead != nil && oldHead.Hash() != newHead.ParentHash {
		...
		if depth := uint64(math.Abs(float64(oldNum) - float64(newNum))); depth <= 64 {
			...
			reinject = pool.pullInAllTransactionsIntoMemory(oldHead, newHead)
			if reinject.Len() == 0 {
				return
			}
		}
...
func (pool *TxPool) pullInAllTransactionsIntoMemory(oldHead, newHead *types.Header) (reinject types.Transactions) {
	...
	return types.TxDifference(discarded, included)
}
...
func (pool *TxPool) truncatePending() { // nolint
	pending := uint64(0)
	for _, list := range pool.pending {
...
func (pool *TxPool) demoteUnexecutables() {
	...
	for addr, list := range pool.pending {
		...
		for _, tx := range invalids {
			...
			pool.enqueueTx(hash, tx) // nolint:errcheck
		}
