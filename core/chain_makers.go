...
func (b *BlockGen) AddTxWithChain(bc *BlockChain, tx *types.Transaction) { // nolint:interfacer
	if b.gasPool == nil {
...
func makeHeader(chain consensus.ChainReader, parent *types.Block, stateDB *state.DB, engine consensus.Engine) *types.Header {
	var time uint64
	if parent.Time() == 0 {
		time = 10
	} else {
