...
func WriteTrace(writer io.Writer, logs []StructLog) {
	for idx := range logs {
		fmt.Fprintf(writer, "%-16spc=%08d gas=%v cost=%v", logs[idx].Op, logs[idx].Pc, logs[idx].Gas, logs[idx].GasCost)
		if logs[idx].Err != nil {
			fmt.Fprintf(writer, " ERROR: %v", logs[idx].Err)
		}
		fmt.Fprintln(writer)

		if len(logs[idx].Stack) > 0 {
			fmt.Fprintln(writer, "Stack:")
			for i := len(logs[idx].Stack) - 1; i >= 0; i-- {
				fmt.Fprintf(writer, "%08d  %x\n", len(logs[idx].Stack)-i-1, math.PaddedBigBytes(logs[idx].Stack[i], 32))
			}
		}
		if len(logs[idx].ReturnStack) > 0 {
			fmt.Fprintln(writer, "ReturnStack:")
			for i := len(logs[idx].Stack) - 1; i >= 0; i-- {
				fmt.Fprintf(writer, "%08d  0x%x (%d)\n", len(logs[idx].Stack)-i-1, logs[idx].ReturnStack[i], logs[idx].ReturnStack[i])
			}
		}
		if len(logs[idx].Memory) > 0 {
			fmt.Fprintln(writer, "Memory:")
			fmt.Fprint(writer, hex.Dump(logs[idx].Memory))
		}
		if len(logs[idx].Storage) > 0 {
			fmt.Fprintln(writer, "Storage:")
			for h, item := range logs[idx].Storage {
