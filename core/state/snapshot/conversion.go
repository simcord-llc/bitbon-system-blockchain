...
func VerifyState(snaptree *Tree, root common.Hash) error {
	...
	got, err := generateTrieRoot(acctIt, common.Hash{}, stdGenerate, func(account common.Hash, stat *generateStats) common.Hash {
		storageIt, err2 := snaptree.StorageIterator(root, account, common.Hash{})
		if err2 != nil {
			return common.Hash{}
		}
		defer storageIt.Release()

		hash, err2 := generateTrieRoot(storageIt, account, stdGenerate, nil, stat, false)
		if err2 != nil {
			return common.Hash{}
		}
		return hash
...
func (stat *generateStats) progress(accounts, slots uint64, curAccount, curSlot common.Hash) {
	stat.lock.Lock()
...
func stdGenerate(in chan trieKV, out chan common.Hash) {
	...
	for leaf := range in {
		t.TryUpdate(leaf.key[:], leaf.value) // nolint
	}
	out <- t.Hash()
}
