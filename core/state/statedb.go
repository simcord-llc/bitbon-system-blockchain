...
func (s *DB) GetLogs(hash common.Hash) []*types.Log {
	return s.logs[hash]
}

func (s *DB) Logs() []*types.Log {
	logs := make([]*types.Log, 0, len(s.logs))
...
func (s *DB) AddRefund(gas uint64) {
...
func (s *DB) SubRefund(gas uint64) {
	s.journal.append(refundChange{prev: s.refund})
	if gas > s.refund {
...
func (s *DB) GetBalance(addr common.Address) *big.Int {
...
func (s *DB) GetNonce(addr common.Address) uint64 {
	stateObject := s.getStateObject(addr)
	if stateObject != nil {
...
func (s *DB) TxIndex() int {
	return s.txIndex
}

// BlockHash returns the current block hash set by Prepare.
func (s *DB) BlockHash() common.Hash {
	return s.bhash
}

func (s *DB) GetCode(addr common.Address) []byte {
	stateObject := s.getStateObject(addr)
	if stateObject != nil {
...
func (s *DB) GetCodeSize(addr common.Address) int {
	stateObject := s.getStateObject(addr)
	if stateObject != nil {
...
func (s *DB) GetCodeHash(addr common.Address) common.Hash {
	stateObject := s.getStateObject(addr)
	if stateObject == nil {
		return common.Hash{}
	}
...
func (s *DB) AddBalance(addr common.Address, amount *big.Int) { // nolint
...
func (s *DB) SubBalance(addr common.Address, amount *big.Int) {
	stateObject := s.GetOrNewStateObject(addr)
	if stateObject != nil {
...
func (s *DB) SetBalance(addr common.Address, amount *big.Int) {
	stateObject := s.GetOrNewStateObject(addr)
	if stateObject != nil {
...
func (s *DB) SetNonce(addr common.Address, nonce uint64) {
	stateObject := s.GetOrNewStateObject(addr)
	if stateObject != nil {
...
func (s *DB) SetCode(addr common.Address, code []byte) {
	stateObject := s.GetOrNewStateObject(addr)
	if stateObject != nil {
...
func (s *DB) SetStorage(addr common.Address, storage map[common.Hash]common.Hash) {
	stateObject := s.GetOrNewStateObject(addr)
	if stateObject != nil {
...
func (s *DB) updateStateObject(obj *stateObject) {
...
func (s *DB) deleteStateObject(obj *stateObject) {
...
func (s *DB) getStateObject(addr common.Address) *stateObject {
...
func (s *DB) getDeletedStateObject(addr common.Address) *stateObject {
...
func (s *DB) GetOrNewStateObject(addr common.Address) *stateObject { // nolint:golint
	stateObject := s.getStateObject(addr)
	if stateObject == nil {
...
func (s *DB) ForEachStorage(addr common.Address, cb func(key, value common.Hash) bool) error {
	so := s.getStateObject(addr)
	if so == nil {
		return nil
	}
...
func (s *DB) Copy() *DB {
	...
	for addr := range s.journal.dirties {
		// As documented [here](https://github.com/simcord-llc/bitbon-system-blockchain/pull/16485#issuecomment-380438527),
...
func (s *DB) RevertToSnapshot(revid int) {
	...
	snapshotIndex := s.validRevisions[idx].journalIndex

	// Replay the journal to undo changes and remove invalidated snapshots
	s.journal.revert(s, snapshotIndex)
	s.validRevisions = s.validRevisions[:idx]
}

// GetRefund returns the current value of the refund counter.
func (s *DB) GetRefund() uint64 {
	return s.refund
}
...
func (s *DB) clearJournalAndRefund() {
...
func (s *DB) Commit(deleteEmptyObjects bool) (common.Hash, error) {
	if s.dbErr != nil {
