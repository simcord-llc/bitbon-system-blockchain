...
func (p *statePrefetcher) Prefetch(block *types.Block, statedb *state.DB, cfg vm.Config, interrupt *uint32) {
	var (
		header  = block.Header()
...
func precacheTransaction(config *params.ChainConfig, bc ChainContext, author *common.Address, gaspool *GasPool, statedb *state.DB, header *types.Header, tx *types.Transaction, cfg vm.Config) error {
	...
	newEVM := vm.NewEVM(context, statedb, config, cfg)

	_, err = ApplyMessage(newEVM, msg, gaspool)
	return err
}
