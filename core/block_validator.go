...
func (v *BlockValidator) ValidateBody(block *types.Block) error {
	...
	if !v.bc.HasBlockAndState(block.ParentHash(), block.NumberU64()-1) {
		if !v.bc.HasBlock(block.ParentHash(), block.NumberU64()-1) {
			log.Error("!v.bc.HasBlock(block.ParentHash(), block.NumberU64()-1)")
...
func (v *BlockValidator) ValidateState(block *types.Block, statedb *state.DB, receipts types.Receipts, usedGas uint64) error {
	header := block.Header()
