...
func (hc *HeaderChain) WriteHeader(header *types.Header) (status WriteStatus, err error) {
	...
	if reorg {
		...
		for i := number + 1; ; i++ {
			hash := rawdb.ReadCanonicalHash(hc.chainDb, i) // nolint
			...
			header := hc.GetHeader(hash, number) // nolint
			if header != nil {
				rawdb.DeleteCanonicalNumberByTime(markerBatch, header.Time)
			}
		}

		// Overwrite any stale canonical number assignments
		hc.overwriteAnyStaleCanonicalNumberAssignments(header, markerBatch, number, hash)
...
// nolint
func (hc *HeaderChain) overwriteAnyStaleCanonicalNumberAssignments(
	header *types.Header, markerBatch ethdb.Batch, number uint64, hash common.Hash) {
	// Overwrite any stale canonical number assignments
	var (
	...
	for rawdb.ReadCanonicalHash(hc.chainDb, headNumber) != headHash {
		...
		rawdb.WriteCanonicalNumberByTime(markerBatch, headerTime, number)
		rawdb.WriteCanonicalHash(markerBatch, headHash, headNumber)

		headHash = headHeader.ParentHash
		headNumber = headHeader.Number.Uint64() - 1
		headHeader = hc.GetHeader(headHash, headNumber)
		headerTime = headHeader.Time
	}
...
// nolint
func (hc *HeaderChain) GetAncestor(hash common.Hash, number, ancestor uint64,
