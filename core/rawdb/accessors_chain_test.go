...
func TestMemRawDb_BlockByTime(t *testing.T) {
	db := NewMemoryDatabase()
	...
	WriteCanonicalNumberByTime(db, timeNow, number)

	numberFromDb := ReadCanonicalNumberByTime(db, timeNow)

	if !assert.NotNil(t, numberFromDb) {
		t.Fatal("return nil block from db")
	}

	assert.Equal(t, &number, numberFromDb)
