...
var (
	...
	ancientMaxFileSize uint32 = 2 * 250 * 1000 * 1000 // ~500 MB
)
...
func newTable(path, name string, readMeter, writeMeter metrics.Meter, sizeGauge metrics.Gauge, disableSnappy bool) (*freezerTable, error) {
	return newCustomTable(path, name, readMeter, writeMeter, sizeGauge, ancientMaxFileSize, disableSnappy)
}
...
func newCustomTable(path, name string, readMeter, writeMeter metrics.Meter, sizeGauge metrics.Gauge, maxFilesize uint32, noCompression bool) (*freezerTable, error) {
...
func (t *freezerTable) repair() error {
	...
	t.index.ReadAt(buffer, 0) // nolint
	firstIndex.unmarshalBinary(buffer)

	t.tailID = firstIndex.offset
	t.itemOffset = firstIndex.filenum

	t.index.ReadAt(buffer, offsetsSize-indexEntrySize) // nolint
...
func (t *freezerTable) Append(item uint64, blob []byte) error {
	...
	if t.headBytes+bLen < bLen ||
		...
		t.openFile(t.headID, openFreezerFileForReadOnly) // nolint
	...
	// Write indexEntry
	t.index.Write(idx.marshallBinary()) // nolint
...
func (t *freezerTable) getBounds(item uint64) (uint32, uint32, uint32, error) { //nolint
