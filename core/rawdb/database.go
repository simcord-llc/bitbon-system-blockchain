...
import (
	...
	"github.com/simcord-llc/bitbon-system-blockchain/ethdb/leveldb"
	"github.com/simcord-llc/bitbon-system-blockchain/ethdb/memorydb"
...
func (db *nofreezedb) AppendAncient(number uint64, hash, header, body, receipts, td, quorumSnapshot []byte) error {
...
// storage.
func NewDatabaseWithFreezer(db ethdb.KeyValueStore, freezer, namespace string) (ethdb.Database, error) {
	...
	if kvgenesis, _ := db.Get(headerHashKey(0)); len(kvgenesis) > 0 {
		...
		} else if ReadHeadHeaderHash(db) != common.BytesToHash(kvgenesis) {
...
func NewLevelDBDatabase(file string, cache, handles int, namespace string) (ethdb.Database, error) {
...
// freezer moving immutable chain segments into cold storage.
func NewLevelDBDatabaseWithFreezer(file string, cache, handles int, freezer, namespace string) (ethdb.Database, error) {
...
func InspectDatabase(db ethdb.Database) error {
	...
	for i, category := range []string{freezerHeaderTable, freezerBodiesTable, freezerReceiptTable, freezerHashTable, freezerDifficultyTable, freezerQuorumTable} {
