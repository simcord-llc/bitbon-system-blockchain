...
type txdata struct {
	...
	Time uint64 `json:"time"     gencodec:"required"` // put new fields to the end of struct for rlp
}

type txdataOrig struct {
	AccountNonce uint64          `json:"nonce"    gencodec:"required"`
	Price        *big.Int        `json:"gasPrice" gencodec:"required"`
	GasLimit     uint64          `json:"gas"      gencodec:"required"`
	Recipient    *common.Address `json:"to"       rlp:"nil"` // nil means contract creation
...
func txdataToOrig(t txdata) txdataOrig {
	return txdataOrig{
...
type txdataMarshaling struct {
	...
	R            *hexutil.Big
	S            *hexutil.Big
	Time         hexutil.Uint64 // timestamp rounded to milliseconds
}
...
func newTransaction(nonce uint64, to *common.Address, amount *big.Int, gasLimit uint64, gasPrice *big.Int, data []byte) *Transaction {
	...
	d := txdata{
		...
		R:            new(big.Int),
		S:            new(big.Int),
		Time:         uint64(time.Now().UTC().UnixNano() / (int64(time.Millisecond) / int64(time.Nanosecond))), // set transaction timestamp rounded to milliseconds
	}
	if amount != nil {
		d.Amount.Set(amount)
	}
...
func isProtectedV(V *big.Int) bool { // nolint
	if V.BitLen() <= 8 {
		v := V.Uint64()
		return v != 27 && v != 28
	}
...
func (tx *Transaction) EncodeRLP(w io.Writer) error {
	if tx.Time() == 0 {
		prev := txdataToOrig(tx.data)
		return rlp.Encode(w, &prev)
	}
	return rlp.Encode(w, &tx.data)
}

var rlpSkipError = "rlp: too few elements for types.txdata"
...
func (tx *Transaction) DecodeRLP(s *rlp.Stream) error {
	...
	if err != nil && err.Error() == rlpSkipError {
		err = nil
	}
...
func (s TxByTime) Less(i, j int) bool { return s[i].data.Time < s[j].data.Time }
...
// transactions in a time-minimizing sorted order, while supporting removing
// entire batches of transactions for non-executable accounts.
