...
type Validator interface {
	...
	// gas used.
	ValidateState(block *types.Block, state *state.DB, receipts types.Receipts, usedGas uint64) error
}
...
type Prefetcher interface {
	...
	Prefetch(block *types.Block, statedb *state.DB, cfg vm.Config, interrupt *uint32)
}
