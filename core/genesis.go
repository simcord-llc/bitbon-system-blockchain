...
func SetupGenesisBlock(db ethdb.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, error) { // nolint
	if genesis != nil && genesis.Config == nil {
		return params.AllEthashProtocolChanges, common.Hash{}, errGenesisNoConfig
	}
	if genesis != nil && genesis.Config != nil && genesis.Config.Quorum != nil {
		if genesis.Config.Quorum.Epoch == 0 {
			return nil, common.Hash{}, fmt.Errorf("missing epoch length field in Quorum section")
		}
		if genesis.Config.Quorum.Start == 0 {
			return nil, common.Hash{}, fmt.Errorf("missing start field in Quorum section")
		}
		if genesis.Config.Quorum.Start%genesis.Config.Quorum.Period != 0 {
			return nil, common.Hash{}, fmt.Errorf("quorum start is not divisable by quorum period")
		}
	}
...
func (g *Genesis) ToBlock(db ethdb.Database) *types.Block {
	...
	statedb.Commit(false)                          // nolint:errcheck
	statedb.Database().TrieDB().Commit(root, true) // nolint:errcheck
