...
func NewApproveFullBalanceWpcSafeTransferParser(ethApiWrapper interfaces.Contract, contractsSnapshot *storage_contract_snapshot.StorageContractSnapshot) *ApproveFullBalanceWpcSafeTransferParser {
	p := NewApproveWPCSafeTransferParser(ethApiWrapper, contractsSnapshot)
	p.MethodId = dto.ApproveFullBalanceWPCSafeTransferV3MethodID

	return &ApproveFullBalanceWpcSafeTransferParser{
		ApproveWPCSafeTransferParser: p,
	}
}
