...
func NewCancelFullBalanceWpcSafeTransferParser(ethApiWrapper interfaces.Contract, contractsSnapshot *storage_contract_snapshot.StorageContractSnapshot) *CancelFullBalanceWpcSafeTransferParser {
	p := NewCancelWPCSafeTransferParser(ethApiWrapper, contractsSnapshot)
	p.MethodId = dto.CancelFullBalanceWPCSafeTransferV3MethodID

	return &CancelFullBalanceWpcSafeTransferParser{
		CancelWPCSafeTransferParser: p,
	}
}
