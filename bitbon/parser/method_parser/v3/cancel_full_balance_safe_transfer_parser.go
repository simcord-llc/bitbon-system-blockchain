...
func NewCancelFullBalanceSafeTransferParser(ethApiWrapper interfaces.Contract, contractsSnapshot *storage_contract_snapshot.StorageContractSnapshot) *CancelFullBalanceSafeTransferParser {
	p := NewCancelSafeTransferParser(ethApiWrapper, contractsSnapshot)
	p.MethodId = dto.CancelFullBalanceSafeTransferV3MethodID

	return &CancelFullBalanceSafeTransferParser{
		CancelSafeTransferParser: p,
	}
}
